...
class RepositorioImagenesSQL(RepositorioImagenes):
    ...
    def obtener_por_estado_procesamiento(self, estado_procesamiento):
        try:
            print("def obtener_por_estado_procesamiento(self, estado_procesamiento):" + estado_procesamiento)
            registros = self.session.query(self._tabla).filter_by(estado_procesamiento=estado_procesamiento).all()
            ...
            raise RepositorioException(f"Error al obtener imagen por tipo estado_procesamiento: {str(e)}")
