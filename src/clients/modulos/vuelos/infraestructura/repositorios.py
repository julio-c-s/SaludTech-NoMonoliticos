...
from clients.modulos.vuelos.dominio.repositorios import RepositorioReservas, RepositorioProveedores
from clients.modulos.vuelos.dominio.objetos_valor import NombreAero, Odo, Leg, Segmento, Itinerario, CodigoIATA
from clients.modulos.vuelos.dominio.entidades import Proveedor, Aeropuerto, Reserva
...
class RepositorioReservasSQLite(RepositorioReservas):
    ...
    def obtener_por_id(self, id: UUID) -> Reserva:
        reserva_dto = db.session.query(ReservaDTO).filter_by(id=str(id)).one_or_none()
        if not reserva_dto:
            return None 
